...
11. Create a tasks.json from the sample provided [here](https://github.com/FLL-Team-24277/Master-Program-Fall-2023/tree/main/.vscode) Save the file in `.vscode\tasks.json`. Edit the file with the robot name (two places). The only changes needed should be to change the label and command argument to match the robot's name. Look for the XXX.
12. Last step, I promise! Add a keyboard shortcut to run the programs that we write. `Ctrl-Shift-P` > `Preferences: Open Keyboard Shortcuts (JSON)`. Edit the JSON to add the keyboard shortcut to run the task. Paste in the code below at the bottom of keybindings.json. Change the args line to match the "label" from tasks.json above. They must match exactly.
